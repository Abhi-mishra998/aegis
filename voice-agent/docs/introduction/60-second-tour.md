...
## Beyond the UI — the Voice Guide

A sibling EC2 hosts the **Aegis Voice Guide**: a LiveKit Agents worker that answers spoken questions against this same documentation. The reader who wanted to skim three pages can instead ask aloud and hear a forty-second answer that quotes the source. Pipeline: Deepgram nova-3 → Groq llama-3.3-70b (Gemini fallback) → Cartesia sonic-3, grounded by hybrid BM25 + dense + cross-encoder retrieval. p50 end-to-end is ~1.3 s in `ap-south-1`. The worker is outbound-only — no inbound port on the EC2 — and registers with LiveKit Cloud, which dispatches sessions to it. See [Voice Guide](../voice-guide/_index.md).
...
- [Voice Guide](../voice-guide/_index.md) — the spoken interface to this documentation, deployed alongside the platform.
