...
The current corpus is 98 markdown files producing 1,704 chunks. Reingesting from scratch takes ~25 s on the EC2 instance — most of it in embedding generation. The operator reruns ingest after any doc edit so the agent's grounding stays current.
...
| Key | Value | Why |
|---|---|---|
| `SYSTEM` | ~4,900-char system prompt | Persona definition: senior security engineer, contractions, short sentences, direct answers, no nanny disclaimers |
...
| `PARAMETER frequency_penalty` | 0.6 | Mitigates v1's "kill switch is… kill switch is…" loops observed before the penalty was tuned |
...
In production session `console-54fc835b` (2026-06-01), the adapter fired during the fourth turn when Groq TPM saturated and routed 2,036 input / 24 output tokens through Gemini. The user did not detect a switch in the audio.
...
- [Deployment and operations](deployment.md) — what the worker process actually looks like in systemd and on EBS, plus cost and failure modes
- [Aegis audit chain](../security/crypto-audit-chain.md) — the same documentation corpus the Voice Guide retrieves over, when read directly
