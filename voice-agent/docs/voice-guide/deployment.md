...
The fix in `voice-agent/agent/src/agent.py:179-185`:
...
Hard cost ceiling per `voice-agent/agent/AGENT_V2.md` §0 is **$250/mo**; realistic operating target is **$15–30/mo**. Three safety nets enforce that:

1. **CloudWatch billing alarm at $200** → SNS topic (email subscription optional).
2. **Auto-stop Lambda** kills the instance after 30 minutes of average CPU below 5%.
3. **Operator workflow assumes start/stop** around demos. The instance is not meant to run 24/7. `infrastructure/scripts/{start,stop,status}.sh` are wrappers around the standard EC2 lifecycle commands plus a wait-for-registration check.
...
| EC2 instance stopped | LiveKit room opens, no agent ever joins | `./infrastructure/scripts/start.sh` | This is by design — auto-stop deliberately stops the instance to save money |
...
| Cold-start latency | First conversation after the box was idle feels sluggish | Pre-warm by hitting the agent once before sharing the demo URL | Documented as part of the demo checklist |
...
2. Opening a session against the LiveKit project hears a one-sentence greeting within 5 seconds of connecting.
3. A doc-grounded question ("what is the kill switch?") produces an answer that uses verbatim Aegis terminology, with a follow-up question that probes a specific dimension.
...
7. CloudWatch shows zero `failed to generate LLM completion after N attempts` errors in the last hour. Sporadic 429s with successful fallback are normal.
8. `journalctl -u aegis-agent.service | grep ERROR | tail -20` shows nothing recent, or only well-understood errors (cold-start prewarm timeouts that recovered).
