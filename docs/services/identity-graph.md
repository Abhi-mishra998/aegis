...
```bash
curl -sS "https://ha.aegisagent.in/graph/agents?limit=200" \
...
```bash
NID=$(curl -sS "https://ha.aegisagent.in/graph/agents?limit=20" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-ID: 00000000-0000-0000-0000-000000000001" \
  | jq -r '.data.nodes[] | select(.name=="devops-agent") | .id')

curl -sS "https://ha.aegisagent.in/graph/blast-radius/$NID?depth=3" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-ID: 00000000-0000-0000-0000-000000000001" \
...
```bash
curl -sS -X POST https://ha.aegisagent.in/graph/compromise/simulate \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-ID: 00000000-0000-0000-0000-000000000001" \
