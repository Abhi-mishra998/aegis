...
| **EH-4** | Supply chain | **Code-shipped** (CI runs on next PR; cosign verify gate on next deploy) | `.github/workflows/security_scan.yml` (Trivy + Gitleaks + Checkov + Bandit); `scripts/ops/sign_bundle.sh` (cosign keyless); user_data now refuses unsigned bundles when `/aegis/prod/require_signed_bundle=true`; `.github/CODEOWNERS` + branch-protection runbook |
...
| **OP-4** | Signed-bundle gate | **Done — bundle signed + uploaded** | `bundle-2026-06-20.tar.gz` signed locally with cosign key-based mode (keyless OIDC requires CI run); `.sig` + `.pem` + `.bundle` uploaded to `s3://aegis-prod-backups-628478946931-v2/releases/`. `verify-blob --bundle … --insecure-ignore-tlog` returns "Verified OK". `/aegis/prod/require_signed_bundle` left at `false` until operator either (a) cuts via GH Actions keyless OR (b) edits `modules/asg/main.tf` to verify with `cosign.pub`. |
...
4. Either run a GH Actions keyless signing build, OR update `modules/asg/main.tf` to verify with `cosign.pub`, then flip `/aegis/prod/require_signed_bundle=true`.
