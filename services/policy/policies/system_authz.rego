...
# N21 defense-in-depth (2026-06-21): exclude `system.*` and `aegis.*` from
# this broad allow so an attacker can never probe the authz rule itself via
# POST evaluate, and can never accidentally query a sensitive package via
# POST. The aegis namespace is opened back up by the explicit allow below.
allow if {
    input.method == "POST"
    input.path[0] == "v1"
    input.path[1] == "data"
...
_path_in_restricted_namespace if {
    input.path[2] in {"system", "aegis"}
}

# 3a. The live decision pipeline DOES POST to evaluate aegis rules
# (e.g. /v1/data/aegis/agent_policy/allow). Allow that explicitly so the
# restricted-namespace guard above doesn't break production traffic.
allow if {
    input.method == "POST"
    input.path[0] == "v1"
    input.path[1] == "data"
