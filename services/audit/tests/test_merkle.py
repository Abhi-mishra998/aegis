...
def test_empty_tree_has_sentinel_root() -> None:
...
def test_every_leaf_has_a_valid_proof(n) -> None:
    leaves = _leaves(n)
...
def test_proof_rejects_wrong_leaf() -> None:
    leaves = _leaves(5)
    proof = inclusion_proof(leaves, 2)
    root = build_root(leaves)
...
def test_proof_rejects_tampered_sibling() -> None:
    leaves = _leaves(5)
    proof = inclusion_proof(leaves, 2)
    root = build_root(leaves)
...
def test_proof_raises_on_malformed_input() -> None:
    with pytest.raises(ValueError, match="missing field"):
        verify_inclusion("a" * 64, {"leaf": "a" * 64}, "b" * 64)


def test_inclusion_proof_index_out_of_range() -> None:
    with pytest.raises(IndexError):
        inclusion_proof(_leaves(3), 10)


def test_inclusion_proof_empty_leaves() -> None:
