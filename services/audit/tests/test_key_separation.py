...
def test_no_env_falls_back_to_receipt_key() -> None:
    r = get_signer()
    root = get_root_signer()
    assert root is r, "must reuse the receipt signer when no separate root key is configured"


def test_separate_root_key_has_different_fingerprint(tmp_path, monkeypatch) -> None:
...
def test_signed_root_uses_root_key_when_separate(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("ROOT_SIGNING_KEY_PATH", str(tmp_path / "root.pem"))
    reset_signer_for_tests()
...
def test_signed_root_verifiable_with_root_key(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("ROOT_SIGNING_KEY_PATH", str(tmp_path / "root.pem"))
    reset_signer_for_tests()
...
def test_signed_root_NOT_verifiable_with_receipt_key_when_separate(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("ROOT_SIGNING_KEY_PATH", str(tmp_path / "root.pem"))
    reset_signer_for_tests()
...
def test_back_compat_root_verifiable_with_same_key_when_no_separate_root(tmp_path) -> None:
