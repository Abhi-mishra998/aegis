...
@pytest.mark.asyncio
async def test_one_pass_commits_each_missing_pair(fake_factory) -> None:
...
@pytest.mark.asyncio
async def test_one_pass_continues_through_per_pair_failures(fake_factory) -> None:
    pairs = [(uuid.uuid4(), date.today() - timedelta(days=1)) for _ in range(3)]

    call_count = {"n": 0}

    async def flaky(db, tenant_id, day) -> None:
        call_count["n"] += 1
        if call_count["n"] == 2:
...
@pytest.mark.asyncio
async def test_scheduler_cancellation_exits_cleanly(fake_factory) -> None:
    import asyncio
    with patch(
...
def test_backfill_window_is_reasonable() -> None:
