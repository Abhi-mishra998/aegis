...
def _verify_sh() -> bytes:
    ...
    return (
        b"#!/usr/bin/env bash\n"
        b"# Verify the SOC 2 evidence bundle with the aegis-verify CLI.\n"
        b"# Requires:  pip install aegis-aevf\n"
        b"set -euo pipefail\n"
        b"echo 'Verifying chain proofs ...'\n"
        b"for f in chain_proofs/*.json; do\n"
        b"  aegis-verify --root \"$f\" --pubkey \"$f.pem\" \\\n"
        b"    || { echo FAIL \"$f\"; exit 1; }\n"
        b"done\n"
        b"echo 'All chain proofs verified.'\n"
        b"echo 'Verifying per-control evidence rows ...'\n"
        b"for f in controls/*.csv; do\n"
        b"  aegis-verify --evidence \"$f\" --chains chain_proofs/ \\\n"
        b"    || { echo FAIL \"$f\"; exit 1; }\n"
        b"done\n"
        b"echo PASS\n"
    )
