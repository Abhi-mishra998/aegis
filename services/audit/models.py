...
class AuditLog(Base, OrgMixin, TenantMixin, IdMixin, TimestampMixin):
    ...
    # `chain_sequence BIGINT GENERATED BY DEFAULT AS IDENTITY` is a
    # DB-side column added by migration z1a2b3c4d5e6. The model
    # deliberately does NOT declare it because the ORM never reads or
    # writes it — external verifiers consume it via raw SQL:
    #
    ...
    #
    # Adding it to the ORM forces SQLAlchemy to introspect the DEFAULT,
    # which on pgbouncer-transaction-mode racy startup can trip
    # asyncpg's prepared-statement cache before our statement_cache_size=0
    # has been threaded through every internal probe. Keep it DB-only.
