...
export default function WebhookSettings() {
  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto space-y-6 animate-pulse" aria-label="Loading webhook settings">
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Unit 9 — empty-state CTA. Visible when nothing is configured. */}
      {nothingConfigured && !loadError && (
        <div className="flex items-start gap-3 p-4 bg-white/[0.02] border border-white/[0.06] rounded-xl">
          <Webhook size={18} className="text-neutral-500 shrink-0 mt-0.5" />
          <div className="flex-1">
            <div className="text-sm font-medium text-neutral-200">No webhooks — add one to forward events</div>
            <p className="text-xs text-neutral-500 mt-1 leading-relaxed">
              Pick a destination below and paste the endpoint URL. Events are signed via HMAC.
              Each row has a <span className="text-neutral-300">Send test</span> button you can fire
              before saving.
            </p>
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              <a href="#generic_url" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/10 text-xs text-neutral-200 hover:border-white/20">
                <Plus size={12} /> Add HTTPS endpoint
              </a>
            </div>
          </div>
        </div>
