...
function generateRego(rules, policyName = 'custom_policy') {
  const lines = [
    `# Generated by Aegis Visual Policy Builder — ${new Date().toISOString().split('T')[0]}`,
    `package ${policyName.replace(/[^a-zA-Z0-9_]/g, '_')}`,
    '',
    'import rego.v1',
    '',
    'default allow := false',
  ...
  rules.forEach((rule, idx) => {
    ...
    const toolCond = rule.conditions.find(c => c.field === 'tool')
    const otherConds = rule.conditions.filter(c => c.field !== 'tool')
    lines.push(`# Rule ${idx + 1}${rule.description ? ': ' + rule.description : ''}`)
    if (action === 'allow') {
      lines.push('allow if {')
    } else {
      lines.push(`${action} if {`)
    }
    if (toolCond) {
      lines.push(`  input.tool_name == "${toolCond.value}"`)
    }
    otherConds.forEach(c => {
      const op = OP_MAP[c.operator] || c.operator
      const val = isNaN(Number(c.value)) ? `"${c.value}"` : c.value
      lines.push(`  input.${c.field} ${op} ${val}`)
    })
    lines.push('}', '')
  })
  return lines.join('\n')
...
function RuleBlock({ rule, idx, onChange, onDelete, onMoveUp, onMoveDown, isFirst, isLast, toolSuggestions = [] }) {
        ...
        <div className="space-y-2">
          ...
          {rule.conditions.map((cond, ci) => (
            <div key={cond.id} className="flex items-center gap-2 flex-wrap">
              ...
              {cond.field === 'tool' && toolSuggestions.length > 0 && (
                <div className="flex flex-wrap gap-1 w-full mt-0.5">
                  {toolSuggestions.map((t) => (
                    <button
                      key={t}
                      type="button"
                      onClick={() => updateCondition(cond.id, 'value', t)}
                      className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-white/10 text-neutral-400 hover:text-white hover:border-white/30 transition-colors"
                    >
                      {t}
                    </button>
                  ))}
...
export default function PolicyBuilder() {
  ...
  const [policyName,          setPolicyName]          = useState('custom_policy')
  const [agentToolSuggestions, setAgentToolSuggestions] = useState([])

  useEffect(() => {
    if (!selectedAgent) { setAgentToolSuggestions([]); return }
    registryService.listPermissions(selectedAgent)
      .then((res) => {
        const perms = res?.data || res || []
        const tools = Array.isArray(perms)
          ? perms.map((p) => p.tool_name || p.resource || p.tool).filter(Boolean) : []
        setAgentToolSuggestions([...new Set(tools)])
      })
      .catch(() => setAgentToolSuggestions([]))
  }, [selectedAgent])
  ...
  const handleTestPolicy = async () => {
    ...
    setError('')
    try {
      const rego = generateRego(rules, policyName)
      const testCases = rules.map((r, i) => ({
        description: r.description || `Rule ${i + 1}`,
        input: {
          tool_name: r.conditions.find(c => c.field === 'tool')?.value || 'test_tool',
          risk_score: parseFloat(r.conditions.find(c => c.field === 'risk_score')?.value ?? '0.8'),
          parameters: {},
        },
        expected_action: r.action.toLowerCase(),
      }))
      const res = await policyService.testPolicy({ rego_code: rego, test_cases: testCases })
  ...
  const handleActivate = async () => {
    if (!policyName.trim()) { setError('Enter a policy name.'); return }
    setActivating(true)
    setActivated(false)
    setError('')
    try {
      const rego = generateRego(rules, policyName)
      await policyService.uploadPolicy({ name: policyName.trim(), rego_code: rego })
      setActivated(true)
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4 min-w-0">
          ...
          {rules.map((rule, idx) => (
            <RuleBlock
              ...
              toolSuggestions={agentToolSuggestions}
            />
          ))}
        ...
        <div className="space-y-4 min-w-0">
          ...
          {/* Rego Preview */}
          {showRego && (
            <Card title="Generated Rego" icon={Code2}>
              <pre className="text-[10px] text-emerald-300/80 bg-black/40 border border-emerald-500/10 rounded-xl p-3 overflow-x-auto leading-relaxed max-h-80 whitespace-pre-wrap font-mono">
                {generateRego(rules, policyName)}
              </pre>
            </Card>
          )}

          {/* Test Policy */}
          <Card title="Test Policy" icon={PlayCircle}>
            <div className="space-y-3">
              <p className="text-[10px] text-neutral-500 leading-relaxed">
                Run your rules through OPA using auto-generated test cases derived from each rule's conditions.
              </p>
              <button
                onClick={handleTestPolicy}
                disabled={testing || rules.length === 0}
                className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs text-blue-400 bg-blue-500/[0.06] border border-blue-500/20 rounded-lg hover:border-blue-500/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                <PlayCircle size={12} aria-hidden="true" />
                {testing ? 'Testing…' : 'Run OPA Tests'}
              </button>

              {testResults && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-[10px]">
                    <span className={testResults.all_passed ? 'text-green-400' : 'text-red-400'}>
                      {testResults.all_passed ? '✓ All tests passed' : '✗ Some tests failed'}
                    </span>
                    <span className="text-neutral-500">
                      {testResults.passed_count}/{testResults.total}
                    </span>
                  </div>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {testResults.results?.map((r, i) => (
                      <div key={i} className={`flex items-center gap-2 p-1.5 rounded text-[10px] border ${r.passed ? 'bg-green-500/[0.04] border-green-500/10' : 'bg-red-500/[0.04] border-red-500/10'}`}>
                        ...
                        {!r.passed && <span className="text-neutral-600 shrink-0">got {r.actual_action}</span>}
                      </div>
                    ))}
                  </div>
                  <button onClick={() => setTestResults(null)} className="text-[10px] text-neutral-600 hover:text-neutral-400 w-full text-center transition-colors">
                    Clear
                  </button>
                </div>
              )}
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="input-standard input-compact h-8 text-xs w-full font-mono"
                />
                <p className="text-[10px] text-neutral-600 mt-1">Letters, numbers, underscores only</p>
              </div>

              {activated && (
                <div className="flex items-center gap-2 p-2.5 rounded-lg bg-green-500/[0.06] border border-green-500/15">
                  <CheckCircle2 size={12} className="text-green-400 shrink-0" aria-hidden="true" />
                  <p className="text-xs text-green-400">Policy uploaded and active.</p>
                </div>
              )}

              <button
                onClick={handleActivate}
                disabled={activating || rules.length === 0 || !policyName.trim()}
                className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs text-amber-400 bg-amber-500/[0.06] border border-amber-500/20 rounded-lg hover:border-amber-500/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
